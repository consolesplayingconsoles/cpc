...
* **Aesthetic**: Channels the vibe of a premium, enthusiast-grade hardware router page — monospaced type, deliberate borders, dense information layout. It skips hollow corporate SaaS decoration — gradients and space for visual effect, elements styled to impress external clients rather than inform power users — in favour of an aesthetic that feels like a physical piece of engineered hardware. Borders, radius, shadows, and subtle colour coding are welcome structural cues and free beauty; they become noise only when used purely as decoration without information value.
* **Function-first modernism**: When a modern design pattern genuinely improves usability or data comprehension — colour-coded status indicators, visual KPI cards, interactive maps, skeleton loading states, loading indicators — prefer it over rigid text-only aesthetics. The goal is *useful beauty*, not retro for its own sake. Clarity beats conformity. The target user is an operator who knows the tool, not a first-time visitor being sold on it.
