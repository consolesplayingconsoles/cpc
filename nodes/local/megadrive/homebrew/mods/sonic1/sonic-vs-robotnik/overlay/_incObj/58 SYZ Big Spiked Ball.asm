...
BBall_Move:	; Routine 2
		; sonic-vs-robotnik: when this ball is Eggman's wrecking ball, objoff_34 points back to
		; the boss. Orbit HIM (re-anchor every frame onto his moving position) instead of a fixed
		; origin. Ordinary level spikeballs leave objoff_34 zero and drop to the vanilla behaviour.
		move.l	objoff_34(a0),d0			; spawned as the boss wrecking ball?
		beq.s	.vanilla				; no -> vanilla level ball
		movea.l	d0,a1					; a1 = boss controller
		move.w	obX(a1),bball_origX(a0)			; re-anchor the orbit onto the moving boss
		move.w	obY(a1),bball_origY(a0)
